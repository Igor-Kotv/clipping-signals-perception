...
import { computeInverseKurtosis } from "./utils";
...
const ExperimentPanel: React.FC = () => {
    ...
    const currentSamplePathArray =
        currentSamples.find((sample) => sample.name === currentSampleName)
            ?.paths || [];
    ...
    const randomIndex =
        sampleCount !== 0 ? shuffledIndexes[sampleCount - 1] : 0;
    ...
    const audioSrc = `${link}${currentSamplePathArray[randomIndex]}`;

    const { audioData: distortedAudio } = useAudioFile(audioSrc);

    const estimate = () => {
        setSampleCount(sampleCount - 1);
        dispatch(evaluateSample({ index: randomIndex, value: score }));
        const averageInverseKurtosis = computeInverseKurtosis(distortedAudio);
        dispatch(
            evaluateSample({
                index: randomIndex,
                value: averageInverseKurtosis,
                isObjective: true,
            }),
        );
